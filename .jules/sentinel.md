...
## 2024-07-07 - [MEDIUM] Information Leakage via Error Handling
**Vulnerability:** Stack traces and platform exceptions were being exposed directly to standard error (`printStackTrace()`) and to the UI via `_showError(e.message)`.
**Learning:** Exposing detailed exception messages or stack traces can leak internal application state and platform details to potential attackers.
**Prevention:** Always log exceptions securely using `android.util.Log.e` or `debugPrint`, and provide generic, safe error messages to the UI.
